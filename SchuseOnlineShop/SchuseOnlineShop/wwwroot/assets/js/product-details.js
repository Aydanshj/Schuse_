...
$(function () {
    ...
    function AddToCart(clickedElem, url) {
        $(document).on("click", clickedElem, function () {
            let productCount = $("#quantityInput").val();
            let id = $(this).attr("data-id");
            let data = { id: id, count: productCount };
...
$(document).on("click", ".dlt", function (e) {
    ...
    $.ajax({
        ...
        success: function () {
            $("button[comment-id='" + id + "']").parent().parent().parent().remove();
        }
    });
});
